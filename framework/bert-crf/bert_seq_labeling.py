...
# added by jerry
flags.DEFINE_bool("do_gt", False, "Whether to run eval on the groundtruth set.")
...
class DataProcessor(object):
    ...
    def _read_data(cls,input_file):
        ...
        for line in rf:
            ...
            # if len(line.strip())==0 and words[-1] == '.': # origin condition
            if len(line.strip()) == 0:  # one example condition of my datasets
                l = ' '.join([label for label in labels if len(label) > 0])
                w = ' '.join([word for word in words if len(word) > 0])
                lines.append((l,w))
                words=[]
                labels = []
            if len(word) and len(label):    # add a condition of appending one example
                words.append(word)
                labels.append(label)
        rf.close()
        return lines
...
class NerProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_example(
            self._read_data(join(data_dir, "train-mag.txt")), "train-mag"
        )

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(join(data_dir, "dev-mag.txt")), "dev-mag"
        )

    def get_test_examples(self,data_dir):
        return self._create_example(
            self._read_data(join(data_dir, "test-mag.txt")), "test-mag"
        )

    def get_gt_examples(self,data_dir):
        return self._create_example(
            self._read_data(join(data_dir, "groundtruth.txt")), "groundtruth"
        )
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        logging.info("\n")
        print(">>>>>>>>>>>>>>> evaluation", "result type {}".format(type(result)))
        print(result)
        ...
        logging.info("***********************************************")
        logging.info("*******************Acc = %s********************",  str(true_num/sum_num))
    ...
    if FLAGS.do_predict:
        ...
        print("\n>>>>>>>>>>>>>>> prediction", "result type {}".format(type(result)))
        print(result)       
    ...
    if FLAGS.do_gt:
        ...
        filed_based_convert_examples_to_features(gt_examples, label_list,
                                            FLAGS.max_seq_length, tokenizer, gt_file)
        ...
        print("\n>>>>>>>>>>>>>>> groundtruth:", "result type {}".format(type(result)))
        print(result)       
